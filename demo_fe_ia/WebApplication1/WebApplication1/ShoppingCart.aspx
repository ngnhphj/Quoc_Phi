...
<asp:DataList ID="db1" runat="server" OnItemCommand="db1_ItemCommand" CssClass="cart-list">
    <ItemTemplate>
        <div class="cart-item">
            <asp:Button ID="btnXoa" runat="server" Text="✕" CommandName="Xoa" CommandArgument='<%# Eval("IDSANPHAM") %>' CssClass="delete-button" />
            <asp:Image ID="Image1" runat="server" AlternateText='<%# Eval("IDSANPHAM") %>' ImageUrl='<%# "images/img_cart/" + Eval("ANHSANPHAM") %>' CssClass="product-image" />
            <asp:Label ID="Label1" runat="server" Text='<%# Eval("TENSANPHAM") %>' CssClass="product-name"></asp:Label>
            <asp:Label ID="Label2" runat="server" Text='<%# Eval("GIASANPHAM") + " $" %>' CssClass="product-price"></asp:Label>
            <asp:Label ID="Label3" runat="server" Text='<%# Eval("GIASANPHAM") + " $" %>' CssClass="product-subtotal"></asp:Label>
            <asp:Button ID="btnMua" runat="server" Text="Buy Now" OnClick="btnMua_Click" CssClass="buy-button" CommandArgument='<%# Eval("IDSANPHAM") %>' />
        </div>
    </ItemTemplate>
</asp:DataList>
